...
int getMin(SinglyList *list, int n, int k)
{
    int sum = 99999999, sumtemp = 0, j = 0;
    while(j+k <= n)
    {
        for(int i = j; i < j+k; i++)
        {
            SListNode *temp2 = list->_head;
            for (int x = 0; x < i; x++) temp2 = temp2->next;
            //printf("%d + %d = %d\n", sumtemp, temp2->data, sumtemp + temp2->data);
            sumtemp += temp2->data;
        }
